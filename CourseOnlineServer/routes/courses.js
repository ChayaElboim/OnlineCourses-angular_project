module.exports = (db) => {
  ...
  const { checkAuth, checkTeacher, checkCourseOwnership } = require('../middlewares/auth')(db);
  ...
  router.post('/', checkAuth, checkTeacher, async (req, res) => {
    ...
    courseModel.create(title, description, teacherId, (err, courseId) => {
      ...
      // After creating, fetch the full course object to return it
      courseModel.findById(courseId, (err, newCourse) => {
        if (err || !newCourse) {
          return res.status(500).json({ message: 'Error fetching newly created course' });
        }
        res.status(201).json(newCourse); // Return the full course object
      });
    });
  });

  // Update course by ID (Teacher only)
  router.put('/:id', checkAuth, checkTeacher, checkCourseOwnership, async (req, res) => {
    const { id } = req.params;
  ...
  router.delete('/:id', checkAuth, checkTeacher, checkCourseOwnership, async (req, res) => {
    const { id } = req.params;
